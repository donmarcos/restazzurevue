...
export default {
  ...
  async configure(clientId, enableDummyUser = true) {
    ...
    const config = {
      auth: {
        ...
        xxxauthority: 'https://login.microsoftonline.com/common',
        authority: 'https://login.microsoftonline.com/9bbbc9f6-c704-4296-8c05-d6d3e87d0b27',
      },
      cache: {
  ...
  async acquireToken(scopes = ['user.read']) {
    ...
    console.log('---> auth acquireToken call');
    ...
    try {
      ...
      console.log('Try to acquire token silently ')
      ...
      console.log('Try to acquire token by pop up')
